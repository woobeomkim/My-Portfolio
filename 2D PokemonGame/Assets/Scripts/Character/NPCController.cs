...
public class NPCController : MonoBehaviour , Interactable, ISavable
{
    ...
    public IEnumerator Interact(Transform initiaor)
    {
        if (state == NPCState.Idle)
        {
            state = NPCState.Dialog;
            character.LookTowards(initiaor.position);

            if(questToComplete != null)
            {
                var quest = new Quest(questToComplete);
                yield return quest.CompleteQuest(initiaor);
                Debug.Log($"{questToComplete.Name} Completed");
            ...
            {
                yield return itemGiver.GiveItem(initiaor.GetComponent<PlayerController>());
            }
            else if(pokemonGiver != null && pokemonGiver.CanBeGiven())
            {
                yield return pokemonGiver.GivePokemon(initiaor.GetComponent<PlayerController>());
            }
            else if (questToStart != null)
            {
                ...
                {
                    yield return activeQuest.CompleteQuest(initiaor);
                    activeQuest = null;
                }
            }
            ...
            { 
                yield return healer.Heal(initiaor, dialog);
            }
